...
samples=($(ls ${PATH1}*_Act.sam | perl -p -e "s/\/.*sam\///g" | perl -p -e "s/_bwa_Act\.sam//g"))
sample=${samples[${SLURM_ARRAY_TASK_ID}]}

samtools view -F 260 ${PATH1}${sample}_bwa_Act.sam | awk '!/^@/ {{ print $0 }}'  | cut -d$'\t' -f3 | sort | uniq -c > ../counts/${sample}_counts.txt